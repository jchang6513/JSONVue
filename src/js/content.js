...
const LABEL_VIEW_PREVIEW = "view preview";
...
let collapserElements, statusElement, jsonObject, copiedSelector, jsonSelector, jsonPath, selectedListItem, hoveredListItem, supportBigInt;
let originalNode, htmlUIParams, currentView = HTML_VIEW
...
async function init() {
	if (document.body && (document.body.childNodes[0] && document.body.childNodes[0].tagName == "PRE" || document.body.children.length == 0)) {
		...
		if (jsonInfo) {
			originalNode = document.body.childNodes[0];
...
async function processData(jsonInfo, options) {
	if ((window == top || options.injectInFrame) && jsonInfo && jsonInfo.text) {
		...
		if (result.html) {
			...
			htmlUIParams = [result.stylesheet, result.html, options]
			try {
				if (supportBigInt) {
...
function displayToolbox(onlyViewSource) {
	...
	viewSourceElement.innerText = currentView === HTML_VIEW ? LABEL_VIEW_SOURCE : LABEL_VIEW_PREVIEW;
...
function onViewSource(event) {
	if (event.isTrusted) {
		if (currentView === HTML_VIEW) {
			document.body.innerHTML = originalNode.outerHTML
			currentView = SOURCE_VIEW
			displayToolbox(true);
		} else {
			displayUI(...htmlUIParams);
